...
public class GetFeatureActivity extends Activity{
	...
	public static final int REQUEST_CODE_ACTIVATE_LICENSE = 1010;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_getfeature);
        setResult(Activity.RESULT_CANCELED);
       
        ((Button) findViewById(R.id.button_activate_feature)).setOnClickListener(btnClick);
        path_EditText = (EditText)findViewById(R.id.editText_pincode_feature);
        ...
        processGroup = (RadioGroup)findViewById(R.id.processGroup_feature);
        sdPathButton = (RadioButton)findViewById(R.id.sdPath_feature);
	    absolutePathButton = (RadioButton)findViewById(R.id.absolutePath_feature);
        ...
        showLogMessage("备注1：存放feature的目录必须保证权限是应用可读可写, 默认存放在SD卡根目录的feature.txt文件里, 用户可以改变存放路径.");
	...
	private View.OnClickListener btnClick = new View.OnClickListener() {
		...
		public void onClick(View v) {
			switch (v.getId()) {
			case R.id.button_activate_feature: {
				int ret = 0;
				...
				ret = Decoder_jni.getInstance().get_feature(getApplicationContext(), finalPath);
				if(ret!=0){
					Log.e(DEBUG_TAG,"get_feature failed,ret="+ret);
					showLogMessage("get_feature failed,ret="+ret);
					break;
				}else{
					Log.e(DEBUG_TAG,"get_feature ok");
					showLogMessage("get_feature ok");
					break;
				}
			}
